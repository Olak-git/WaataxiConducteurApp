...
const DrawerMenu: React.FC<DrawerMenuProps> = ({ navigation, containerStyle, iconType = 'font-awesome-5', iconName, iconSize = 25, textMenu, screenName, screenParams={}, disabled }) => {
    ...
    return (
        <TouchableOpacity
            ...
            <Text style={[ tw`px-4 text-xl ${disabled ? 'text-gray-300' : 'text-gray-500 text-black'} text-base`, {fontFamily: 'YanoneKaffeesatz-Medium'}, {fontFamily: polices.times_new_roman} ]}>{ textMenu }</Text>
        </TouchableOpacity>
    )
}
